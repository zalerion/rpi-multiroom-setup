...
sudo mount -t cifs  $mount /mnt/share -o noserverino,user=$user,password=$pwd
...
echo "sudo mount -t cifs  $mount /mnt/share -o noserverino,user=$user,password=$pwd" > /etc/automount.sh
