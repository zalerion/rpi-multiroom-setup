...
echo
sudo mount -t cifs  $mount /mnt/share -o noserverino,vers=3.0,user=$user,password=$pwd

read -p "Want to make it permanent? Please be aware, that your password will be saved in plaintext (y/N)" go

if [[ ! "$go" =~ ^(yes|y|Y)$ ]]; then exit 0; fi

echo "sudo mount -t cifs  $mount /mnt/share -o noserverino,vers=3.0,user=$user,password=$pwd" > /etc/automount.sh
...
sudo cat  <<EOM > /etc/playlistBackup.sh
	...
	sudo cp /mnt/share/Playlists/Playlists/* /mnt/share/Playlists/"Backup_Playlists_\$(date +"%Y_%m_%d")"
EOM
